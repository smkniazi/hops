...
//      BlockReport dnBr = getDNBR(cluster, dn, numBuckets);
//      List dnHashes = new ArrayList();
//      for (long hash : dnBr.getHashes()) {
//        dnHashes.add(hash);
//      }
//      List<Long> nnHashes = getNameNodeHashes(cluster, nnId/*nn*/, dn, numBuckets);
//
//      Collections.sort(nnHashes); //sorts by index
//      Collections.sort(dnHashes);
//
//      assertTrue("Size of NN and DN hashes do not match. NN Size: " + nnHashes
//              .size() + " DN Size:" + dnHashes.size(), nnHashes.size() == dnHashes
//              .size());
//
...
//        Long dnHash = (Long) dnHashes.get(j);
//        Long nnHash = nnHashes.get(j);
//        if (!dnHash.equals(nnHash)) {
////          LOG.debug("Hashes do not match. Index: " + j + " NN Hash: " +
////                  nnHash +
////                  " DN " + "Hash:" + dnHash);
//          mismatchCount++;
//        }
...
//      HashBuckets.getInstance().corruptHashBuckets(dnd);
//    }
//  }
//
...
//      BlockManager bm = cluster.getNamesystem(nnId).getBlockManager();
//      DatanodeDescriptor dnd = bm.getDatanodeManager().getDatanode(dn.getDatanodeId());
//      HashBuckets.getInstance().deleteHashBuckets(dnd);
//    }
//  }
//  private BlockReport getDNBR(MiniDFSCluster cluster, DataNode dn, int
//          numBuckets) {
//    String poolId = cluster.getNamesystem().getBlockPoolId();
//    BlockReport br = dn.getFSDataset().getBlockReport(poolId);
//    assertEquals("Wrong number of buckets read for DN: " + dn, numBuckets,
//            br.getBuckets().length);
//    return br;
//  }
//
//  private void sendAndCheckBR(int nnId, int numDataNodes, MiniDFSCluster cluster,
//                              String poolId, int tollerence, int numBuckets)
//          throws
//          IOException {
//    int mismatched = 0;
//    for (int i = 0; i < numDataNodes; i++) {
...
//      BlockReport dnBr = getDNBR(cluster, dn, numBuckets);
//      BlockManager bm = cluster.getNamesystem(nnId).getBlockManager();
//      ReportStatistics stats = bm.processReport(dn.getDatanodeId(), poolId,
//              dnBr);
...
//          nnId, DataNode dn, int numBuckets) throws IOException {
//    // all blocks belong to the same file, hence same BP
//
//    BlockManager bm = cluster.getNamesystem(nnId).getBlockManager();
//    DatanodeDescriptor dnd = bm.getDatanodeManager().getDatanode(dn.getDatanodeId());
//    List<HashBucket> namenodeHashes = HashBuckets.getInstance()
//            .getBucketsForDatanode(dnd);
//    assertEquals("Wrong number of buckets on the NN Size for DN "
//                    + dn.getDatanodeId(), numBuckets, namenodeHashes.size());
//
