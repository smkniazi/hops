...
//      make sure that the blocks are deleted from the disk
  ...
  public void blockReport_10() throws IOException, InterruptedException {
    concrrentWrites(5 /*threads*/,
            (short) 3 /*replication*/,
            10 /*numDataNodes*/,
            0 /*threshold*/);
  }

  public void concrrentWrites(int numThreads, short replication, int
  ...
  private void matchDNandNNState(int nnId, int numDataNodes, MiniDFSCluster
          cluster, int tollerance, int numBuckets) throws IOException {
    ...
    for (int i = 0; i < numDataNodes; i++) {
      ...
      Map<DatanodeStorage, BlockReport> map = getDNBRs(cluster, dn, numBuckets);
      for(DatanodeStorage nds : map.keySet()){
        BlockReport report = map.get(nds);
        long[] dnHashes = report.getHashes();

        DatanodeStorageInfo dnSId = dnd.getStorageInfo(nds.getStorageID());
        List<Long> nnHashes = getNameNodeHashes(cluster, nnId/*nn*/, dnSId, numBuckets);

        assertTrue("Size of NN and DN hashes do not match. NN Size: " + nnHashes
                .size() + " DN Size:" + dnHashes.length, nnHashes.size() == dnHashes.length);

        LOG.debug("DN (SId"+dnSId.getSid()+") Hash: " + Arrays.toString(dnHashes));
        LOG.debug("NN (SId"+dnSId.getSid()+") Hash: " + Arrays.toString(nnHashes.toArray()));

        for (int j = 0; j < numBuckets; j++) {
          long dnHash = dnHashes[j];
          long nnHash = nnHashes.get(j);
          if (dnHash != nnHash) {
//          LOG.debug("Hashes do not match. Index: " + j + " NN Hash: " + nnHash + " DN " + "Hash:" + dnHash);
            mismatchCount++;
          }
        }
      }
  ...
  private void corruptHashes(int nnId, MiniDFSCluster cluster) throws IOException {
    for (DataNode dn : cluster.getDataNodes()) {
      ...
      for (DatanodeStorageInfo sInfo : dnd.getStorageInfos()) {
        HashBuckets.getInstance().corruptHashBuckets(sInfo);
      }
    }
  }
  ...
  private void deleteHashes(int nnId, MiniDFSCluster cluster) throws IOException {
    for (DataNode dn : cluster.getDataNodes()) {
      ...
      for (DatanodeStorageInfo sInfo : dnd.getStorageInfos()) {
        HashBuckets.getInstance().deleteHashBuckets(sInfo);
      }
    }
  }

  private Map<DatanodeStorage, BlockReport> getDNBRs(MiniDFSCluster cluster, DataNode dn, int
          numBuckets) {
    String poolId = cluster.getNamesystem().getBlockPoolId();
    Map<DatanodeStorage, BlockReport> map = dn.getFSDataset().getBlockReports(poolId);
    for(BlockReport br : map.values()){
      assertEquals("Wrong number of buckets read for DN: " + dn, numBuckets, br.getBuckets().length);
    }
    return map;
  }

  private void sendAndCheckBR(int nnId, int numDataNodes, MiniDFSCluster cluster,
                              String poolId, int tollerence, int numBuckets) throws IOException {
    int mismatched = 0;
    ...
    for (int i = 0; i < numDataNodes; i++) {
      ...
      for (DatanodeStorage nds : map.keySet()) {
        BlockReport report = map.get(nds);
        ReportStatistics stats = bm.processReport(dn.getDatanodeId(), nds, report);
        mismatched += (numBuckets - stats.getNumBucketsMatching());
      }
    }

    if (mismatched > tollerence) {
      String msg = "BR Buckets mismatched : " + mismatched + " Tollerence: " +
              tollerence;
      LOG.debug(msg);
      fail(msg);
    }
  ...
  private int countNNBlocks(int nnId, MiniDFSCluster cluster) throws IOException {
    ...
    return count;
//    return cluster.getNamesystem(nnId).getBlockManager() .getActiveBlockCount();
  }

  private List<Long> getNameNodeHashes(MiniDFSCluster cluster, int
          nnId, DatanodeStorageInfo storage, int numBuckets) throws IOException {
    // all blocks belong to the same file, hence same BP

    BlockManager bm = cluster.getNamesystem(nnId).getBlockManager();
    List<HashBucket> namenodeHashes = HashBuckets.getInstance().getBucketsForDatanode(storage);
    assertEquals("Wrong number of buckets on the NN Size for DN "
            + storage.getSid(), numBuckets, namenodeHashes.size());

    Collections.sort(namenodeHashes);
    List<Long> nnHashes = new ArrayList();
    for (HashBucket hashBucket : namenodeHashes) {
      nnHashes.add(hashBucket.getHash());
    }
    return nnHashes;
  }
